...
class VnmFilter < VNMMAD::VNMDriver
    ...
    def initialize(vm_template, xpath_filter = nil, deploy_id = nil)
        @locking = true
        @slog = Syslog::Logger.new 'vnm_filter'
        ...
        nics.each do |nic_id, nicdata|
            ...
            if nic[:filter_ip_spoofing] == "YES"
                ...
                if iptables_s !~ /#{chain}-ip-spoofing/
                    @slog.info "altering #{chain_o} to add #{chain}-ip-spoofing"
                    commands.add :ipset, "create -exist #{chain}-ip-spoofing hash:ip family inet"
                    commands.add :iptables, "-R #{chain_o} 2 -m set ! --match-set #{chain}-ip-spoofing src -j DROP"
                    commands.add :iptables, "-I #{chain_o} 2 -s 0.0.0.0/32 -d 255.255.255.255/32 -p udp -m udp --sport 68 --dport 67 -j RETURN"
                end
                ...
                if ip6tables_s !~ /#{chain}-ip6-spoofing/
                    ...
                    commands.add :ip6tables, "-R #{chain_o} 5 -m set ! --match-set #{chain}-ip6-spoofing src -j DROP"
                end
